...
int esferas_duras(int a);//nuevas funciones
...
float variacion_energia(void){
	...
	//condicion 1
	//	ecoul_i+=cte*ion[0].carga*1/(sqrt(pow((ion[0].x-ancho/2),2)+pow((ion[0].y-ancho/2),2)+pow((ion[0].z-ancho/2),2))*esc);//en estas dos lineas se pone una carga positiva de 1 carga electrica fundamental en el centro de la celda (10,10,10) la interaccion tiene simetria esferica
	//	ecoul_f+=cte*ion[ni].carga*1/(sqrt(pow((ion[ni].x-ancho/2),2)+pow((ion[ni].y-ancho/2),2)+pow((ion[ni].z-ancho/2),2))*esc);
	//condicion 2
	//	ecoul_i+=cte*ion[0].carga*1/(sqrt(pow((ion[0].x-ancho/2),2)+pow((ion[0].y-ancho/2),2))*esc);//en estas dos lineas se pone una carga positiva de 1 carga electrica fundamental en el centro de la celda (10,10,10) la interaccion tiene simetria cilindrica
	//	ecoul_f+=cte*ion[ni].carga*1/(sqrt(pow((ion[ni].x-ancho/2),2)+pow((ion[ni].y-ancho/2),2))*esc);
	//condicion 3	
		ecoul_i+=cte*ion[0].carga*1e-2/(sqrt(pow((ion[0].z-ancho/2),2))*esc);//en estas dos lineas se pone una carga positiva de 1 carga electrica fundamental en el centro de la celda (x=10) la interaccion tiene simetria plana
		ecoul_f+=cte*ion[ni].carga*1e-2/(sqrt(pow((ion[ni].z-ancho/2),2))*esc);		
	}
	den=ecoul_f-ecoul_i;
	//printf("Den=%1.6e\n",den);
	return(den);
...
float distancia(int a, int b){
	...
	yy=ion[b].y;
	zz=ion[b].z;
	if (fabs(ion[a].x-xx)>ancho/2)xx+=ancho*signo(ion[a].x-xx);//check backwards
	if (fabs(ion[a].y-yy)>ancho/2)yy+=ancho*signo(ion[a].y-yy);
	if (fabs(ion[a].z-zz)>ancho/2)zz+=ancho*signo(ion[a].z-zz);
...
int esferas_duras(int a){
	int i, res=1;
	for(i=1; i<=a-1; i++)if(distancia(i,a)<diam)return(res);
	res=0;
	return(res);
}
/////////////////////////////////////////////////////////////////////////////THE END///////////////////////////////////////////////////////////////////////
